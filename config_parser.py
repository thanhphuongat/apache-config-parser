    ...
    def __str__(self):
        if not self.lines:
            raise NodeCompleteError(
                "Can't turn an uninitialized comment node into a string.")
        if self.changed:
    ...
    def add_line(self, line):
        if line.endswith("\\"):
            raise InvalidLineError(
                "Blank lines cannot have line continuations.")
...
class Directive(Node):
        ...
        def __set__(self, obj, value):
            if hasattr(obj, "_name") and obj._name is not None:
                raise DirectiveError(
                    "Name is already set.  Cannot set to %s" % value)
                ...
                class ValidatedArgs(ListAdapter):
                    ...
                    def validate_general(self, val):
                        ...
                        if not re_mod.match("'.*'$|\".*\"$", val) and ("<" in val or ">" in val):
                            raise DirectiveError(
                                "Angle brackets not allowed in directive args.  Received: %s" % val)
    ...
    def add_to_header(self, line):
        if self.complete:
            raise NodeCompleteError(
                "Cannot add to the header of a complete directive.")
        if not line:
    ...
    def complete(self, val):
        if val and not self.stable:
            raise NodeCompleteError(
                "Can't set an unstable Directive to complete.")
...
class SimpleDirective(Directive):
    ...
    def __str__(self):
        if not self.lines:
            raise NodeCompleteError(
                "Can't turn an uninitialized simple directive into a string.")
                            ...
                            for item in self.items:
                                if isinstance(item, Directive):
                                    ...
                                    if item not in self._directives[item.name]:
                                        self._directives[item.name].append(
                                            item)
    ...
    def add_line(self, line, depth=0):
        ...
        if depth > self.NESTING_LIMIT:
            raise NestingLimitError(
                "Cannot nest directives more than %s levels." % self.NESTING_LIMIT)
    ...
    def add_line(self, line, depth=0):
        ...
        if not self.header.complete:
            ...
            if ">" in line:
                ...
                if remainder.strip() != "":
                    raise InvalidLineError(
                        "Directive header has an extraneous tail: %s" % line)
            else:
                header = line
